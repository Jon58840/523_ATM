...
public class CheckPinPM {
	...
	public static void checkPIN(int currentAccountNumber, Monitor m, KeypadInterface keypad, AccountDatabase db,
			SysTimer sysClock) {
		...
		if(pin == db.getAccount(currentAccountNumber).getPIN()) {
			...
			if(SCB.getPinTrialTimes() > 0) {
				m.showMessages("Wrong PIN.", "Do you want to try again?");
